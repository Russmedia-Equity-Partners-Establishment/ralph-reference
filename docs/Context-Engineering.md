...
For example, in the Todo tool, just listing status as an enumeration between `pending`, `in_progress`, and `completed` hints to Claude about how to use it. The instruction on keeping one item `in_progress` helps define our requested behavior.
...
This might be in specs files, mockups, or even entire codebases. Generally you should prefer files that are in code, as it provides clear, high-fidelity instructions to Claude in a language it knows very well. For example, an HTML mockup of a design will generally produce better results than a description of the design or a screenshot.
